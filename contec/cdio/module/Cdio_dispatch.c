...
#define		MEM_DIO_PORT_ADD	(dio_data->bm_board.dwAddr_mem+0xB000000)			// The base port address for memory map        (I/O)
...
int Ccom_register_entry(CCOM_ENTRY *ccom_entry)
{
	...
	ccom_entry->dispatch			= Cdio_dispatch;			// Entry for dispatch
	ccom_entry->init_resource_type	= Cdio_init_resource_type;	// Entry for resource type acquisition
...
int Cdio_add_device(void **pdevice_ext, PCCOM_FILE_DATA fdata)
{
	...
	int				itype;
	long			lret;
	unsigned long	vendor_id, device_id, board_id, device_type;
	void			*device_ext;
	PCDIO_DATA		dio_data;
	...
	unsigned long	irq;
	unsigned long	minor;
	unsigned long	intdata, intdata2;
	unsigned long	mem_data, mem_data2, mem_data3;
	unsigned char	board_id_mem, board_rev_mem, rom_rev_mem, unique_info_mem;
	unsigned short	Command;
	unsigned int	setlegacy;
...
//	init_MUTEX(&dio_data->mutex_flag);
	...
	// Device type
	lret = Ccom_read_value(fdata, "", "DeviceType", &itype, (void *)&device_type);
	if (lret != CCOM_NO_ERROR) {
		dio_data->g_ret = DIO_ERR_INI_REGISTRY;
		return 0;
	}
...
#endif
	Cdio_init_resource_type(dio_data);
	//----------------------------------------
	...
	mem_num	= 6;
	//--------------------------------------
	//	If it is not DEMO device
	//--------------------------------------
	if (device_type != DEVICE_TYPE_DEMO) {
		//----------------------------------------
		// If the BusMaster library is supported in the kernel
		//----------------------------------------
#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0) && LINUX_VERSION_CODE < KERNEL_VERSION(2,7,0)) ||\
	...
	//------------------------------------------------------------------------
	//The FPGA used by the DIO-32DM3-PE disables interrupts by default.
	//To enable the interrupt, it's necessary to set 0 to the bit10 of Command register 
	//and the 16bit of Configuration register + 0x50h.
	//------------------------------------------------------------------------
	if (strcmp(dio_data->board_name, "DIO-32DM3-PE") == 0) {
		pci_read_config_word(PciDev, PCI_COMMAND, &Command);
		Command		= Command & 0xFBFF;
		pci_write_config_word(PciDev, PCI_COMMAND, Command);
		pci_read_config_dword(PciDev, 0x050, &setlegacy);
		setlegacy	= setlegacy & 0xFFFEFFFF;
		pci_write_config_dword(PciDev, 0x050, setlegacy);
	}
	//----------------------------------------
	// Initialize the BIOS
	//----------------------------------------
	if (strcmp(dio_data->board_name, "DIO-32DM3-PE") == 0){
		dio_data->phy_mem_base		= mem_addr[0];
		dio_data->phy_bm_mem_base	= mem_addr[1];
		dio_data->mem_base			= (unsigned char *)ioremap(mem_addr[0], 0xfffffff);
		dio_data->bm_mem_base		= (unsigned char *)ioremap(mem_addr[1], 0xfffffff);
		dio_data->board_id	= board_id;
	} else {
		dio_data->port_base	= (unsigned short)io_addr[0];
		dio_data->board_id	= board_id;
	}
	if (dio_data->resource_type == SRC_TYPE_MEMIO){
		mem_data	= _inmw(dio_data->mem_base + 0xB000054);
		mem_data2	= _inmw(dio_data->mem_base + 0xB000056);
		if ((mem_data == 0x1221) && (mem_data2 == 0xA642)) {
			//--------------------------------------
			// Save board ID to extension
			//--------------------------------------
			mem_data3					= _inmw(dio_data->mem_base + 0xB000050);
			board_id_mem				= mem_data3 & 0xff;
			dio_data->board_id			= board_id_mem;
			//--------------------------------------
			// Save board revision (Unique_info_No)
			//--------------------------------------
			mem_data3					= _inmw(dio_data->mem_base + 0xB000050);
			unique_info_mem				= (mem_data3 & 0xff00) >> 8;
			dio_data->board_revision	= unique_info_mem;
			//--------------------------------------
			// Get board revision (Board_rev_No) and ROM revision
			//--------------------------------------
			mem_data3					= _inmw(dio_data->mem_base + 0xB000052);
			rom_rev_mem					= (mem_data3 & 0xff);
			board_rev_mem				= (mem_data3 & 0xff00) >> 8;
			//---------------------------------------
			// Initialize BIOS
			//----------------------------------------
			lret	= Cdiobi_init_bios_mem(&dio_data->bios_d, dio_data->board_name, dio_data->mem_base, irq);
		} else {
			//--------------------------------------
			// Save board ID to extension
			//--------------------------------------
			mem_data3					= _inmw(dio_data->bm_mem_base + 0xB000050);
			board_id_mem				= mem_data3 & 0xff;
			dio_data->board_id			= board_id_mem;
			//--------------------------------------
			// Save board revision (Unique_info_No)
			...
			// Get board revision (Board_rev_No) and ROM revision
			//--------------------------------------
			mem_data3					= _inmw(dio_data->bm_mem_base + 0xB000052);
			rom_rev_mem					= (mem_data3 & 0xff);
			board_rev_mem				= (mem_data3 & 0xff00) >> 8;
			//----------------------------------------
			// Initialize BIOS
			//----------------------------------------
			lret	= Cdiobi_init_bios_mem(&dio_data->bios_d, dio_data->board_name, dio_data->bm_mem_base, irq);
		}
		//--------------------------------------
		// Save board revision (Board_rev_No) and ROM revision
		//--------------------------------------
		dio_data->bios_d.bi_rom_revision	= rom_rev_mem;
		dio_data->bios_d.bi_board_version	= board_rev_mem;
	} else {
		lret = Cdiobi_init_bios(&dio_data->bios_d, dio_data->board_name, io_addr, irq);
		//--------------------------------------
		// Save board revision (Board_rev_No) and ROM revision
		//--------------------------------------
		dio_data->bios_d.bi_rom_revision	= dio_data->rom_revision;
		dio_data->bios_d.bi_board_version	= dio_data->board_version;
	}
	if (lret != 0) {
	...
	if (device_type != DEVICE_TYPE_DEMO) {
		...
		if(dio_data->bios_d.b_info->board_type == CDIOBI_BT_PCI_DM){
			if (strcmp(dio_data->board_name, "DIO-32DM3-PE") == 0) {
				...
				//----------------------------------------
				sprintf(dio_data->display_name, "%s_%ld", dio_data->board_name, board_id);
				lret	= Ccom_report_io_mem(dio_data->phy_mem_base, 0xfffffff, dio_data->display_name);
				if (lret != CCOM_NO_ERROR) {
					dio_data->g_ret	= DIO_ERR_INI_RESOURCE;
					return 0;
				}
				lret	= Ccom_report_io_mem(dio_data->phy_bm_mem_base, 0xfffffff, dio_data->display_name);
				if (lret != CCOM_NO_ERROR) {
					dio_data->g_ret	= DIO_ERR_INI_RESOURCE;
					return 0;
				}
				dio_data->bm_board.dwAddr_mem	= dio_data->mem_base;
				dio_data->bm_board.dwAddrBM_mem	= dio_data->bm_mem_base;
				if ((unsigned char *)mem_addr[1] != NULL){
					intdata		= _inmd(dio_data->bm_board.dwAddrBM_mem + 0x050);
					intdata2	= intdata;
					if ((intdata2 & 0xffff) != 0xffff){
						intdata	|= 0xffff;
						_outmd((dio_data->bm_board.dwAddrBM_mem + 0x050), intdata);
					}
				} else {
					dio_data->g_ret = DIO_ERR_INI_RESOURCE;
					return 0;
				...
				//----------------------------------------
				lret = ApiMemBmCreate(&dio_data->bm_board.Master, PciDev, dio_data->bm_board.dwAddrBM_mem, dio_data->bm_board.dwAddr_mem);
				...
				//----------------------------------------
				sprintf(dio_data->board_name, "%s_%ld", dio_data->board_name, board_id);
				lret	= Ccom_report_io(dio_data->bm_board.dwAddrBM, 64, dio_data->board_name);
				if (lret != CCOM_NO_ERROR) {
					dio_data->g_ret	= DIO_ERR_INI_RESOURCE;
					return 0;
				}
		...
		if (irq != 0xffffffff) {
			...
			tasklet_init(&dio_data->tasklet, Cdio_dm_trans_end_process, (unsigned long)dio_data);
		}
	}
	return 0;
}
...
int Cdio_delete_device(void *device_ext)
{
	...
	//----------------------------------------
	if(dio_data->bios_d.b_info->board_type != CDIOBI_BT_DEMO){
		Ccom_disconnect_interrupt(&dio_data->isr_obj);
	}
	if (strcmp(dio_data->board_name, "DIO-32DM3-PE") == 0){
		if (dio_data->bm_mem_base == NULL){
			return -ENODEV;
		}
		intdata		= _inmd(dio_data->bm_mem_base + 0x050);
		intdata		&= 0xffff0000;
		_outmd((dio_data->bm_mem_base + 0x050), intdata);
	}
	//----------------------------------------
	// If the BusMaster library is supported in the kernel
	//----------------------------------------
	...
	if(dio_data->bios_d.b_info->board_type == CDIOBI_BT_PCI_DM){
		if (strcmp(dio_data->board_name, "DIO-32DM3-PE") == 0){
			if (dio_data->bm_mem_base != NULL){
				ApiMemBmDestroy(&dio_data->bm_board.Master);
				Ccom_unreport_mem(dio_data->phy_bm_mem_base, 0xfffffff);
			}
		}
		else {
	...
	if(dio_data->bios_d.b_info->board_type != CDIOBI_BT_DEMO){
		...
		//----------------------------------------
		if (strcmp(dio_data->board_name, "DIO-32DM3-PE") == 0){
			Ccom_unreport_mem(dio_data->phy_mem_base, 0xfffffff);
		}
		else {
	...
	//----------------------------------------
	if (strcmp(dio_data->board_name, "DIO-32DM3-PE") == 0){
		iounmap(dio_data->bm_mem_base);
		iounmap(dio_data->mem_base);
	}
	...
	if (strcmp(dio_data->board_name, "DIO-32DM3-PE") == 0){
		...
		if (dio_data->ref_count == 0) {
			...
			if(dio_data->bios_d.b_info->board_type == CDIOBI_BT_PCI_DM){
				...
				//--------------------------------------------
				if (dio_data->bm_mem_base != NULL){
					ApiMemBmStop(&dio_data->bm_board.Master, BM_DIR_IN | BM_DIR_OUT);
				}
				//---------------------------------------------
				// Unlock the momery
				//---------------------------------------------
				ApiMemBmUnlockMem(&dio_data->bm_board.Master, BM_DIR_IN);
				ApiMemBmUnlockMem(&dio_data->bm_board.Master, BM_DIR_OUT);
				//----------------------------------------
				// Mask all the interrupts
				//----------------------------------------
...
//////////////////////////////////////////////////////////////////////////////////////////
///	@brief	Entry for resource type acquisition
///	@param	board_name		Device name
///	@param	resource_type	Store resource type
///	@return	NTSTATUS
//////////////////////////////////////////////////////////////////////////////////////////
int Cdio_init_resource_type(void *device_ext)
{
	long			lret;
	PCDIO_DATA		dio_data;

	//----------------------------------------
	// Cast device information structure for DIO
	//----------------------------------------
	dio_data	= (PCDIO_DATA)device_ext;
	//----------------------------------------
	// Call subroutine
	//----------------------------------------
	lret = Cdiobi_init_resource_type(dio_data->board_name, &dio_data->resource_type);
	if (lret != 0) {
		dio_data->g_ret = DIO_ERR_INI_RESOURCE;
		return 0;
	}
...
int Cdio_dispatch(void *device_ext, unsigned int ctl_code, void *param)
{
	...
	case CDIO_IOC_DM_SET_BUF:					// Buffer setting
		...
		//----------------------------------------
		dio_data = (PCDIO_DATA)device_ext;
		dio_data->bm_board.Master.add64flag	= 0;
#if defined(__x86_64__)
		dio_data->bm_board.Master.add64flag	= 1;
#endif
	...
	case CDIO_IOC_DM_SET_COUNT_EVENT:			// Notification of the transfer completion with specified number setting
		...
		kfree(param_copy);
		break;
	case CDIO_IOC_GET_CUR_DEV_INFO:				// Information acquisition function for initialized device
		param_copy	= kmalloc(sizeof(CDIO_GET_CUR_INFO), GFP_KERNEL);
		iret		= copy_from_user(param_copy, (PCDIO_GET_CUR_INFO)param, sizeof(CDIO_GET_CUR_INFO));
		lret		= Cdio_get_current_device_info(device_ext, (PCDIO_GET_CUR_INFO)param_copy);
		iret		= copy_to_user((PCDIO_GET_CUR_INFO)param, param_copy, sizeof(CDIO_GET_CUR_INFO));
		kfree(param_copy);
		break;
#endif
	default:
...
long Cdio_init(void *device_ext, PCDIO_INIT param)
{
	...
	if(dio_data->bios_d.b_info->board_type == CDIOBI_BT_PCI_DM){
		if (strcmp(dio_data->board_name, "DIO-32DM3-PE") == 0){
			if (dio_data->bios_d.b_info->device_id != 0xE001){
				//----------------------------------------
				...
				dio_data->bm_board.SavePioSence				= 0x0;
				// Slave signal state initialization
				...
				dio_data->bm_board.Sampling.InternalClock		= 200;
				...
				dio_data->bm_board.Generating.InternalClock		= 200;
				...
				dio_data->bm_board.Master.BmInp.dmaactflag			= 0;
				dio_data->bm_board.Master.BmOut.dmaactflag			= 0;
				dio_data->bm_board.Master.BmOut.resetcountFlag			= 0;
				dio_data->bm_board.Master.BmInp.resetcountFlag			= 0;
				//---------------------------------------------------------
				// Set input/output direction to [PI_32]
				//---------------------------------------------------------
				dio_data->bm_board.direction				= PI_32;
				//---------------------------------------------------------
				// Set wait time at generating start to 10ms
				//---------------------------------------------------------
				dio_data->bm_board.WaitTimeWhenOutput		= 10;
				//----------------------------------------
				// Reset all
				//----------------------------------------
				// Initialization of PIO start flag
				dio_data->bm_board.KeepStart				= 0;
				ApiMemBmReset(&dio_data->bm_board.Master, BM_RESET_ALL);
				_outmd(MEM_DIO_PORT_ADD + 0x14, 0xffffffff);		// Interrupt flag
			}
		}else{
			//----------------------------------------
			// Only one process can be used
	...
	//----------------------------------------
	// For the internal power supply devices of CONPROSYS 
	// Turn on internal power supply
	//----------------------------------------
	Cdiobi_power_enable(&dio_data->bios_d);
	//----------------------------------------
	// Release the spinlock
	//----------------------------------------
	spin_unlock_irqrestore(&dio_data->spinlock_flag, flags);
...
long Cdio_exit(void *device_ext, PCDIO_EXIT param)
{
	...
	wait_queue_head_t	wq;			// Wait queue (2,6,0)
#endif
	unsigned long		flags;
...
long Cdio_reset(void *device_ext, PCDIO_RESET param)
{
	...
	Cdiobi_set_all_int_mask(&dio_data->bios_d, CDIOBI_MASK_ALL, NULL);
	//----------------------------------------
	// If the memory I/O device is reset during DMA transfer, 
	// the status may not be correctly initialized, so reinitialize it.
	//----------------------------------------
	if (strcmp(dio_data->board_name, "DIO-32DM3-PE") == 0){
		_outmd(dio_data->bm_board.dwAddr_mem+0xB000014, 0xFFFFFFFF);
		dio_data->bm_board.Master.BmOut.dwIntSence		= 0;
		dio_data->bm_board.Master.BmOut.resetcountFlag	= 1;
		dio_data->bm_board.Master.BmInp.resetcountFlag	= 1;
	}
	//----------------------------------------
	// Release the spinlock
	//----------------------------------------
...
long Cdio_set_interrupt_event(void *device_ext, PCDIO_SET_INT param)
{
	...
	//----------------------------------------
	// Check supoort boards
	//----------------------------------------
	if (strcmp(dio_data->bios_d.b_info->board_name, "DIO-CPS-BXC200") == 0){
...
//////////////////////////////////////////////////////////////////////////////////////////
///	@brief	Information acquisition function of initialized device (private)
///	@param	device_ext		Device information
///	@param	param			Control structure
///	@return	NTSTATUS
///	@note	The Cdio_get_device_info () function uses the device name to 
///			get information from the device information structure, 
///			but this function uses the device ID obtained by DioInit() to get information associated with the device ID.
//////////////////////////////////////////////////////////////////////////////////////////
int Cdio_get_current_device_info(void *device_ext, PCDIO_GET_CUR_INFO param)
{
	PCDIO_DATA	dio_data;
	long		lret = DIO_ERR_SUCCESS;
	//----------------------------------------
	...
	// Call subroutine
	//----------------------------------------
	lret = Cdiobi_get_cur_info(&dio_data->bios_d,  param->type, &param->param1, &param->param2, &param->param3);
	//----------------------------------------
	// Set return value
	//----------------------------------------
	param->ret = 0;
