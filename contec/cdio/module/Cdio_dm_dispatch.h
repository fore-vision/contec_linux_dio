...
typedef	struct {
	...
	unsigned char* 	dwAddr_mem;			// Base address (PIO)
	unsigned char*	dwAddrBM_mem;		// Base address (BM)
	...
	unsigned long	SavePioSence;		// Interrupt sense storage (for MemDevice)
	unsigned long	SavePioMask;		// Interrupt mask storage (for MemDevice)
	...
	unsigned long	dwdir;				// Input/Output setting
} BMBOARD, *PBMBOARD;
