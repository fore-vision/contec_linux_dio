...
static long pci_read_revision(struct pci_dev *ppci_dev, unsigned char* board_id, unsigned char* board_revision);

//========================================================================
// Function name  : pci_read_revision
// Function 	  : Get the Board ID and the Board Revision of the PCI device.
// I/F	 		  : Internal
// In	 		  : ppci_dev		: Pointer to the PCI device structure
	...
	struct pci_dev *ppci_dev)
{
	...
	*irq		= 0xffffffff;
	...
	while ((pdev = pci_find_device(vendor_id, device_id, pdev))) {
		...
		//----------------------------------------
		if (pci_read_revision(pdev, &real_board_id, &board_revision) != 0) {
				return -1;
		}
	    ...
	    (LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)))
			// The index of I/O and memory is not necessarily continuous.
			// example:  0 -> I/O   2 -> memory
			// If the index is 1, it will be broke, and the memory cannot be retrieved .
...
void ApiBmSetStopEvent(PMASTERADDR pMasAddr, unsigned long dwDir)
{
	if (dwDir == BM_DIR_OUT) {
		...
		pMasAddr->BmOut.dwIntSence |= BM_STATUS_ALL_END;
