...
int	Ccom_open(struct inode *inode, struct file *file)
{
	...
	//--------------------------------------
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0) && LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0))
	...
	for (ul=0; ul<file_len; ul++) {
		...
		if (file_data[ul] == '\n') {
			...
			case '}':		// Block end
				...
				// If the device type is DEVICE_TYPE_PCI or DEVICE_TYPE_DEMO, add into the device list.
				lret	= Ccom_read_value(ptmp, "", "DeviceType", &itype, (void *)&device_type);
				if ((device_type == DEVICE_TYPE_PCI) || (device_type == DEVICE_TYPE_DEMO))
				{
...
// Report I/O address
//================================================================
long Ccom_report_io_mem(unsigned long port, unsigned long range, char *name)
{
	//--------------------------------------
	// Check the parameters
	//--------------------------------------
	...
	// Report
	//--------------------------------------
	request_mem_region(port, range, name);
	return CCOM_NO_ERROR;
}
...
// Unreport I/O address
//================================================================
long Ccom_unreport_mem(unsigned long mem, unsigned long range)
{
	release_mem_region(mem, range);
	return CCOM_NO_ERROR;
}
