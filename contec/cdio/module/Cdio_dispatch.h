...
typedef struct {
	...
	unsigned long			phy_mem_base;							// Physical address of memory
	unsigned long			phy_bm_mem_base;						// Physical address of memory
	unsigned char*			mem_base;								// Base address of memory
	unsigned char*			bm_mem_base;							// Base address of memory
	unsigned short			port_num;								// Number of occupied ports
	unsigned short			mem_num;
	unsigned short			irq_no;									// IRQ
	unsigned long			board_id;								// Board ID
	unsigned char			board_revision;							// Revision (Unique_info_No)
	unsigned char			rom_revision;							// Rom revision (ROM_rev_No)
	unsigned char			board_version;							// Board revision (Board_rev_No)
	unsigned char			resource_type;							// Resource type (1: Not use, 2: Board I/O, 3: Memory I/O)
