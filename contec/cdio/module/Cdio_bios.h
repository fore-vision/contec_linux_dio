...
#define	CDIOBI_BT_PCI_CONPROSYS		0x02000000	// Board type: CONPROSYS series
#define	CDIOBI_BT_DEMO				0x00010000	// Board type: DEMO device
// I/O board: ASIC
...
// I/O board: BusMaster DIO-32DM3-PE
#define	CDIOBI_PORT_DM3_INTMASK		0xB000010	// Interrupt mask
#define	CDIOBI_PORT_DM3_INTSTS		0xB000014	// Interrupt status
// I/O board: CONPROSYS series
#define	CDIOBI_PORT_CONPROSYS_FILTER	0x19	// Digital filter
#define	CDIOBI_PORT_CONPROSYS_INTMASK	0x20	// Interrupt mask
#define	CDIOBI_PORT_CONPROSYS_INTSTS	0x20	// Interrupt status
#define	CDIOBI_PORT_CONPROSYS_INTCTRL	0x24	// Interrupt control
...
typedef struct {
	...
	unsigned short	resource_type;							// Revision Type
	unsigned short	port_num;								// Number of occupied ports
	unsigned long	inp_port_off;							// Offset of input port address
	unsigned long	out_port_off;							// Offset of output port address
	unsigned long	filter_off;								// Offset of filter address
	unsigned long	int_mask_off;							// Offset of interrupt mask address
	unsigned long	int_sence_off;							// Offset of interrupt sence address
	unsigned long	int_ctrl_off;							// Offset of interrupt control address
...
typedef struct {
	...
	unsigned char *		port_base_mem;						// Base address of port
	...
	unsigned char *		inp_port_mem;						// Input port address
	unsigned char *		out_port_mem;						// Output port address
	unsigned char *		filter_port_mem;					// Filter address
	unsigned char *		int_mask_port_mem;					// Interrupt mask address
	unsigned char *		int_sence_port_mem;					// Interrupt sence address
	unsigned char *		int_ctrl_port_mem;					// Interrupt control address
	unsigned char		out_data[CDIOBI_MAX_PORT];			// Output latch data
	unsigned char		bi_rom_revision;					// Rom revision (ROM_rev_No)
	unsigned char		bi_board_version;					// Board revision (Board_rev_No)
	...
	unsigned long		direction;							// Input/output setting
} CDIOBI, *PCDIOBI;
//================================================================
// Function Prototype
//================================================================
