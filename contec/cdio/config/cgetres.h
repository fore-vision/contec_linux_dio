...
typedef struct {
	...
	unsigned long		device_id;		// Device ID
	unsigned long		board_id;		// Board ID
	unsigned long 		subsys_id;		// SUBSYSTEM ID
	long				ret;			// Return value (It is the same as the return code)
} GETPCISUBSYS, *PGETPCISUBSYS;
...
long get_pci_subsys(PGETPCISUBSYS param);
#endif
//---------------------------------------------------------------
...
#define	CGETSUBSYS_IOC_GET_PCI		_IOWR(CGETRES_IOC_MAGIC, 3, PGETPCISUBSYS)	// Get the PCI subsystem ID
