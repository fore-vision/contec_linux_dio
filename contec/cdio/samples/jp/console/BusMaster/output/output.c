...
// 外部変数
//================================================================
static char				file_name[] = "output.txt";	// 保存ファイル名
static unsigned int		data_buff[DATA_SIZE];		// データバッファ
static unsigned long    is_ring;					// 1回転送 or 無限回転送
...
int main()
{
	...
	while(1){
		...
		case MENU_DATA_SET:
			...
			//-----------------------------------
			fclose(fp);
			sprintf(buf, "%s からデータを設定しました。", file_name);
			TermSetStatus(STATUS_AREA_BOTTOM, STS_RET_POS, buf);
			break;
		//--------------------------------------------
		// 転送スタート
		...
		case MENU_START:
			...
			// Set Buffer
			//----------------------------
			lret = DioDmSetBuff(id, dir, (unsigned long *)data_buff, DATA_SIZE, is_ring);
			if (lret != DIO_ERR_SUCCESS){
				DioGetErrorString(lret, get_str);
