...
// 外部変数
//================================================================
static unsigned long		read_index;						// リードポインタ
static unsigned long		write_index;					// ライトポインタ
static unsigned long		pre_count;						// 前回カウント値
static unsigned long		pre_carry;						// 前回キャリー値
static char					file_name[] = "infinite.txt";	// 保存ファイル名
static unsigned int			data_buff[DATA_SIZE];			// データバッファ
...
int main()
{
	...
	while(1){
		...
		case MENU_START:
			...
			// Set Buffer
			//----------------------------
			memset(data_buff, 0, DATA_SIZE * sizeof(unsigned int));
			lret = DioDmSetBuff(id, dir, (unsigned long *)data_buff, DATA_SIZE, DIODM_WRITE_RING);
			if (lret != DIO_ERR_SUCCESS){
				DioGetErrorString(lret, get_str);
			...
			// Start
			//----------------------------
			read_index	= 0;	// リードポインタ
			write_index	= 0;	// ライトポインタ
			pre_count	= 0;	// 前回カウント値
			pre_carry	= 0;	// 前回キャリー値
