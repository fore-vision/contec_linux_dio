...
static unsigned int		data_buff_master[DATA_SIZE_MASTER];			// Master data buffer
static unsigned int		data_buff_slave[DATA_SIZE_SLAVE];			// Slave data buffer
...
int master()
{
	...
	while(1){
		...
		case MENU_START:
			...
			// Set Buffer
			//----------------------------
			memset(data_buff_master, 0, DATA_SIZE_MASTER * sizeof(unsigned int));
			lret = DioDmSetBuff(id_master, dir, (unsigned long *)data_buff_master, DATA_SIZE_MASTER, is_ring_master);
			if (lret != DIO_ERR_SUCCESS){
				DioGetErrorString(lret, get_str);
...
int slave()
{
	...
	while(1){
		...
		case MENU_START:
			...
			// Set Buffer
			//----------------------------
			memset(data_buff_slave, 0, DATA_SIZE_SLAVE * sizeof(unsigned int));
			lret = DioDmSetBuff(id_slave, dir, (unsigned long *)data_buff_slave, DATA_SIZE_SLAVE, is_ring_slave);
			if (lret != DIO_ERR_SUCCESS){
				DioGetErrorString(lret, get_str);
				sprintf(buf, "DioDmSetBuff %ld:%s", lret, get_str);
				TermSetStatus(STATUS_AREA_BOTTOM, STS_RET_POS, buf);
