	...
	for(count = 0; strcmp(set_scr->input_num[count].set_item_name, "") && count < SET_ITEM_MAX_COUNT;  count++){
		...
		if(pnum_data->hex_or_dec == DEC_NUM){
			if(strcmp(pnum_data->unit_name, "")){
				printf("%-20s: %d [%s]\n", pnum_data->set_item_name, (int)pnum_data->set_num, pnum_data->unit_name);
			}else{
				printf("%-20s: %d\n", pnum_data->set_item_name, (int)pnum_data->set_num);
			}
		//--------------------------------------
		...
		//--------------------------------------
			...
			if(strcmp(pnum_data->unit_name, "")){
				printf("%-20s: %X [%s]\n", pnum_data->set_item_name, (unsigned int)pnum_data->set_num, pnum_data->unit_name);
			}else{
				printf("%-20s: %X\n", pnum_data->set_item_name, (unsigned int)pnum_data->set_num);
			}
		}
	}
	//--------------------------------------
...
long TermGetInputNum(PSET_SCR_DATA set_scr, int item_num, unsigned long *input_num)
{
	...
	if(set_scr->input_num[item_num].hex_or_dec == DEC_NUM){
		scanf("%d", (int *)input_num);
	// Hexadcimal
	}else{
		scanf("%x", (unsigned int *)input_num);
	}
	//--------------------------------------
