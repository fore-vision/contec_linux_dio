...
// External variable
//================================================================
static char				file_name[] = "input.txt";	// Name of saving file 
static unsigned int		data_buff[DATA_SIZE];		// Data Buffer
...
int main()
{
	...
	while(1){
		...
		case MENU_START:
			...
			lret = DioDmSetBuff(id, dir, (unsigned long *)data_buff, DATA_SIZE, DIODM_WRITE_ONCE);
			if (lret != DIO_ERR_SUCCESS){
				DioGetErrorString(lret, get_str);
		...
		case MENU_DATA:
			...
			//----------------------------
			fclose(fp);
			sprintf(buf, "Save data to file: %s", file_name);
			TermSetStatus(STATUS_AREA_BOTTOM, STS_RET_POS, buf);
			break;
		//--------------------------------------------
